...
```{r, eval = TRUE, echo = FALSE, fig.show = "asis", fig.width=15, fig.height=5, result = "hide"}
# Histogram dy in dependcy of the slope
inputPanel(
        selectInput("bi", label = "Number of bins:", choices = c(500, 750, 1000, 2000), selected = 500),
        sliderInput("dy_hist_range", label = "Range:", min = -10*10^6, max = 10*10^6, value = c(-200000,500000))
)
renderPlot(
        dyHistogram(mydata, low = as.numeric(input$dy_hist_range[1]), up = as.numeric(input$dy_hist_range[2]), same_scale = TRUE, logscale = FALSE, bi = as.numeric(input$bi))
)
#dyHistogram(mydata, low = -5000000, up = -200000, same_scale = TRUE, logscale = FALSE, bi =100)
```
...
inputPanel(
        selectInput("bi", label = "Number of bins:", choices = c(500, 750, 1000, 2000), selected = 500),
        sliderInput("slope_hist_range", label = "Change slope window:", min = -10*10^6, max = 10*10^6, value = c(-200000,500000))
)
renderPlot(
        SlopeHistogram(mydata,low = as.numeric(input$slope_hist_range[1]), up = as.numeric(input$slope_hist_range[2]), logscale = FALSE, set_x_marker = 0, bi = as.numeric(input$bi), pr = 0, w = 3, h = 1)
)
#SlopeHistogram(mydata, logscale = FALSE, set_x_marker = 0, bi = 500, low = 0, up = 0, pr = 0, w = 3, h = 1)
```


```{r, eval = TRUE, echo = TRUE, fig.width=10, fig.height=8}
## Plot Slopes
PlotSlope_1(mydata, logscale = TRUE, c = 2)
```


```{r, eval = TRUE, echo = TRUE, fig.width=10, fig.height=8}
## Plot Slopes
PlotSlope_2(mydata, logscale = TRUE, c = 2)
```


```{r, eval = TRUE, echo = TRUE, fig.width=10, fig.height=22}
## Plot Slopes
PlotSlope(mydata, c = 2, w = 4, h = 10)
```
